...
@test "run release script from develop in quiet mode" {
	git checkout develop
	echo "some work" >> somefile
	git add somefile
	git commit -m "Do some work"

	[[ -z $(./release-scripts/release.sh --quiet 23.1 23.2) ]]
...
@test "run hotfix from develop in quiet mode" {
	./release-scripts/release.sh 23.1 23.2
	git push --atomic origin master develop --follow-tags

	# ------------------------- START HOTFIX -------------------------

	[[ -z $(./release-scripts/hotfix_start.sh --quiet 23.1.1) ]]
  ...
  [[ -z $(./release-scripts/hotfix_finish.sh --quiet 23.1.1 23.2) ]]
